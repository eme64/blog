...
*Definition Aliasing*: two memory accesses are said to alias if ther accessed memory regions overlap.
...
between the iterations. The implication is that the iterations are decidedly not parallelizable, and so naive vectorizaton
...
Note that we may need multiple such checks if there are multiple pairs of possibly aliasing memmory references.
Some examples:
